...
<?php
		...
		foreach ($fields as $field) {
			if (strpos($action, 'add') !== false && $field == $primaryKey) {
				...
				if($this->templateVars['schema'][$field]['null'] === false && $field != $primaryKey){
					echo "\t\techo \$this->Form->input('{$field}', array('afterInput'=>'<span class=\"help-block\"><span class=\"label label-warning\">'".".__('Required')."."'</span></span>'));\n";
				} else {
					echo "\t\techo \$this->Form->input('{$field}');\n";
				}
			}
